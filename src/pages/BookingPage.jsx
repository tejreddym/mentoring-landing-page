...
const BookingPage = ({ onClose, isModal = false }) => {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            days.push(
                <div
                    ...
                    onClick={() => status !== 'booked' && handleDateSelect(dateKey)}
                >
    ...
    return (
        <div
            id="booking-page"
            className={`min-h-screen flex items-center justify-center ${isModal ? 'p-0 bg-transparent' : 'p-4 bg-slate-50'}`}
        >
            <div
                className={`rounded-3xl overflow-hidden max-w-3xl w-full flex flex-col md:flex-row min-h-[500px] ${isModal ? 'bg-white shadow-2xl' : 'bg-white shadow-2xl'}`}
            >
                <div className={`w-full md:w-1/3 p-8 flex flex-col ${isModal ? 'bg-slate-50 border-r border-slate-100' : 'bg-slate-50 border-r border-slate-100'}`}>
                    <button onClick={goBack} className="mb-8 flex items-center gap-2 text-slate-500 hover:text-slate-800 font-medium transition-colors">
                        ...
                        {step === 3 && (
                            <div ref={step3Ref} className="h-full flex flex-col items-center justify-center text-center relative px-4">
                                ...
                                <div className="flex flex-col gap-3 w-full max-w-xs mx-auto">
                                    <button
                                        onClick={() => navigate('/taara-dashboard')}
                                        className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg shadow-blue-500/20 hover:shadow-xl hover:scale-105 transition-all text-sm"
                                    >
                                        Go to Dashboard
                                    </button>
                                    <button
                                        onClick={() => (isModal && onClose ? onClose() : navigate('/mentor-profiles'))}
                                        className="w-full py-3 bg-white border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-50 transition-colors text-sm"
                                    >
                                        Back to Mentors
                                    </button>
                                </div>
